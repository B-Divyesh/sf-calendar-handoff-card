...
The completed repair was pushed to `origin/main` at `4d8ec53` (`45feb41..4d8ec53`). As of `2026-08-29T00:43Z`, the live custom domain still serves the previous artifact: `/not-a-real-page` returns 200 with the old home title and the old stage labels. I attempted the work-order static deployment after a successful local build using both `calendar-handoff-card` and the DNS-resolved Static Web App name `icy-glacier-06f41f70f`; `swa deploy … --env production --no-use-keychain` stalled during Azure resource resolution without a deployment result. A direct management-plane query with the supplied workload identity also found no Static Web App in the permitted subscription whose name or properties match `calendar-handoff-card`. The work order exposes no deployment token or target resource-group mapping, so this cannot be resolved safely from the repository. No secrets were written or exposed. A successful factory deployment must be followed by the live route/404 cold check specified above.
